...
#SBATCH --output="job.PR_n43.%J.out"
#SBATCH --job-name="PR_VCF_n43"
#SBATCH --array=1-13
...
cat AD1_PR_n43.txt | sentieon driver --interval Ah_$seq -t $thr -r $ref --algo GVCFtyper --emit_mode all $TMPDIR/$output1.Ah_$seq.vcf -
mv $TMPDIR/$output1.Ah_$seq.vcf* $Dir/$output1/

cat AD1_PR_n43.txt | sentieon driver --interval Dh_$seq -t $thr -r $ref --algo GVCFtyper --emit_mode all $TMPDIR/$output1.Dh_$seq.vcf -
