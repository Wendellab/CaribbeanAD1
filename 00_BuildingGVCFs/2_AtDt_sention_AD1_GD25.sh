...
#SBATCH --output="job.GD_n25.%J.out"
#SBATCH --job-name="GD_VCF_n25"
#SBATCH --array=1-13
...
cat AD1_GD_n25.txt | sentieon driver --interval Ah_$seq -t $thr -r $ref --algo GVCFtyper --emit_mode all $TMPDIR/$output1.Ah_$seq.vcf -
mv $TMPDIR/$output1.Ah_$seq.vcf* $Dir/$output1/

cat AD1_GD_n25.txt | sentieon driver --interval Dh_$seq -t $thr -r $ref --algo GVCFtyper --emit_mode all $TMPDIR/$output1.Dh_$seq.vcf -
